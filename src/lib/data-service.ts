...
export async function getMosqueSettings(): Promise<MosqueSettings> {
  if (isSupabaseConnected() && supabase) {
    try {
      const { data, error } = await supabase.from('pengaturan').select('*').limit(1).single();
      if (!error && data) return data as MosqueSettings;
    } catch (e) {
      console.warn('Supabase fetch settings failed, using fallback');
    }
  }
...
export async function updateMosqueSettings(settings: MosqueSettings): Promise<MosqueSettings> {
  if (isSupabaseConnected() && supabase) {
    try {
      ...
      console.warn('Supabase update settings error, updating fallback');
    }
  }
...
export async function getCategories(): Promise<Category[]> {
  if (isSupabaseConnected() && supabase) {
    try {
      ...
      console.warn('Supabase fetch categories error');
    }
  }
...
export async function addCategory(category: Omit<Category, 'id'>): Promise<Category> {
  const newCat: Category = {
    ...category,
    id: isSupabaseConnected() ? undefined! : `cat-${Date.now()}`,
  };
  ...
  if (isSupabaseConnected() && supabase) {
    try {
      ...
      console.warn('Supabase category add error');
    }
  }
...
export async function updateCategory(category: Category): Promise<Category> {
  if (isSupabaseConnected() && supabase && category.id && !category.id.startsWith('cat-')) {
    try {
      ...
      console.warn('Supabase update category error');
    }
  }
...
export async function getTransactions(): Promise<Transaction[]> {
  if (isSupabaseConnected() && supabase) {
    try {
      ...
      console.warn('Supabase transactions fetch error');
    }
  }
...
export async function addTransaction(trx: Omit<Transaction, 'id' | 'trx_code'>): Promise<Transaction> {
  ...
  if (isSupabaseConnected() && supabase) {
    try {
      ...
      console.warn('Supabase insert transaction error');
    }
  }
...
export async function updateTransaction(trx: Transaction): Promise<Transaction> {
  if (isSupabaseConnected() && supabase && trx.id && !trx.id.startsWith('trx-')) {
    try {
      ...
      console.warn('Supabase update transaction error');
    }
  }
...
export async function cancelTransaction(id: string): Promise<boolean> {
  if (isSupabaseConnected() && supabase && !id.startsWith('trx-')) {
    try {
      ...
      console.warn('Supabase cancel transaction error');
    }
  }
...
export async function getUsers(): Promise<User[]> {
  if (isSupabaseConnected() && supabase) {
    try {
      ...
      console.warn('Supabase users fetch error');
    }
  }
...
export async function addUser(user: Omit<User, 'id'>): Promise<User> {
  ...
  if (isSupabaseConnected() && supabase) {
    try {
      ...
      console.warn('Supabase add user error');
    }
  }
...
export async function updateUser(user: User): Promise<User> {
  if (isSupabaseConnected() && supabase && user.id && !user.id.startsWith('u-')) {
    try {
      ...
      console.warn('Supabase update user error');
    }
  }
...
export async function uploadReceiptFile(file: File): Promise<string> {
  if (isSupabaseConnected() && supabase) {
    try {
      ...
      console.warn('Supabase upload file error, using base64 fallback');
    }
  }
